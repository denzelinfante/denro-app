...
export default function FormA() {
  ...
  const [rows, setRows] = useState<Row[]>([
    { label: 'Roads', selected: false, date: '' },
    { label: 'Water systems', selected: false, date: '' },
    { label: 'Transmission lines', selected: false, date: '' },
    { label: 'Telecommunication/ communication facilities', selected: false, date: '' },
    { label: 'Heavy equipment', selected: false, date: '' },
    { label: 'Irrigation Facilities', selected: false, date: '' },
  ]);
  ...
  const handleNext = async () => {
    ...
    if (picked.length === 0) {
      Alert.alert('Nothing selected', 'Please select at least one facility.');
      return;
    }
    const summary = picked
      .map(r => (r.date ? `${r.label} (${r.date})` : r.label))
      .join(', ');
    await addSubmission({ form: 'A. Major Facilities', note: summary });
    router.push('/FormB');
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.formBox}>
        <Text style={styles.formTitle}>A. Major Facilities</Text>
        <Text style={styles.formNote}>
          Check as applicable and as observed and date established
        </Text>
...
const styles = StyleSheet.create({
  ...
  dateInput: {
    borderBottomWidth: 1, borderColor: '#000', width: 100, textAlign: 'center',
